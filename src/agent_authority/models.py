"""Core contracts for identity, authority, policy, execution, and proof."""
...
class AuthorityToken(BaseModel):
    ...
    def active(self) -> bool:
        return not self.revoked and utcnow() < self.expires_at and self.action_count < self.max_actions and (
            self.max_cost <= 0 or self.spent_cost <= self.max_cost
        )
