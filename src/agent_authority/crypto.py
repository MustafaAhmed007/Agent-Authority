...
class Signer:
    ...
    def sign(self, payload: object) -> str:
        return base64.urlsafe_b64encode(self.private_key.sign(canonical_json(payload))).decode()

    def verify(self, payload: object, signature: str) -> bool:
        try:
            self.public_key.verify(base64.urlsafe_b64decode(signature.encode()), canonical_json(payload))
            return True
