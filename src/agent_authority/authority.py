...
class Authority:
    ...
    def _capability_matches(self,t:AuthorityToken,r:AuthorizationRequest)->Capability|None:
        for c in t.capabilities:
            if (not c.operations or any(fnmatch.fnmatch(r.action,str(p)) for p in c.operations)) and (not c.resources or any(fnmatch.fnmatch(r.resource,str(p)) for p in c.resources)): return c
        return None
